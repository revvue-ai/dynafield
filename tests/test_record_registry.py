...
def test_build_and_fetch_records_from_registry():
    ...
    typed_records = registry.build_records(schema.schema_id, stored_records)

    assert len(typed_records) == 2
    assert typed_records[0].customer_name == "Ada"
    assert typed_records[1].party_size == 2

    fetched_schema = registry.get_for_table("bookings", schema.schema_id)
...
def test_mutate_records_with_registry():
    ...
    mutated_records = registry.mutate_records(schema.schema_id, stored_records, updates)
